...
bool tcp_try_coalesce(struct sock *sk, struct sk_buff *to, struct sk_buff *from)
{
	...
	if (tcp_hdr(from)->fin)
		return false;
	if (!skb_try_coalesce(to, from, &delta))
		return false;
...
int __must_check tcp_queue_rcv(struct sock *sk, struct sk_buff *skb, int hdrlen)
{
	...
	eaten = (tail &&
		 tcp_try_coalesce(sk, tail, skb)) ? 1 : 0;
...
int tcp_send_rcvq(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	if (tcp_queue_rcv(sk, skb, sizeof(*th)))
		__kfree_skb(skb);
	return size;
		     ...
		     struct sock *sk, struct sk_buff *skb)
{
	...
	if (want_cookie) {
		...
		if (tmp_opt.saw_tstamp && tcp_death_row.sysctl_tw_recycle) {
			...
			dst = af_ops->route_req(sk, &fl, req, &strict, want_cookie);
			if (dst && strict &&
	...
	if (!dst) {
		dst = af_ops->route_req(sk, &fl, req, NULL, true);
		if (!dst)
			goto drop_and_free;
	}
	...
	err = af_ops->send_synack(sk, dst, &fl, req,
				  skb_get_queue_mapping(skb), true, &foc);
	if (!fastopen) {
		if (err || want_cookie)
			goto drop_and_free;
