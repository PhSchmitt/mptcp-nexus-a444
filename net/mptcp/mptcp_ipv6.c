					    ...
					    bool *strict, bool nocache)
{
	...
	if (sk->sk_family == AF_INET6)
		return tcp_request_sock_ipv6_ops.route_req(sk, fl, req, strict, nocache);
	...
	dst = ip6_dst_lookup_flow(sk, &fl6, NULL, nocache);
	if (IS_ERR(dst))
		return NULL;
	return dst;
}
				  ...
				  u16 queue_mapping)
{
	...
	fl6.fl6_dport = inet_rsk(req)->ir_rmt_port;
	fl6.fl6_sport = inet_rsk(req)->ir_num;
	security_req_classify_flow(req, flowi6_to_flowi(&fl6));
